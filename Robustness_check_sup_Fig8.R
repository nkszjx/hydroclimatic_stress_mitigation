...
# Robustness checks: Alternative spatial aggregation levels
...
urban_in <- merged_dt[urban == "in"]
urban_out <- merged_dt[urban == "out"]
cont_vars <- c(
  "FVC","NPP","WaterDeficit","Seasonalityindex","AverageTemperature","TemperatureRange","Precipitation",
  "WindSpeed","SoilMoisture","SoilPH","Elevation","Latitude",
  "GDP_per_capita_PPP","HDI","Population","ImperviousSurface","HumanSettlement","CityArea"
)
other_vars <- c("CountryID","continent","country","ClimateZone","DevelopmentLevel",
                "expansionrate","CitySize","urban")
dt_mean <- urban_out[, lapply(.SD, mean, na.rm = TRUE),
                     by = .(CityID, year),
                     .SDcols = cont_vars]
dt_first <- urban_out[, lapply(.SD, function(x) x[1L]),
                      by = .(CityID, year),
                      .SDcols = other_vars]
city_year_dt <- merge(dt_mean, dt_first, by = c("CityID", "year"))
keep_cols <- c("CityID","year", cont_vars, other_vars)
urban_in_sub <- urban_in[, ..keep_cols]
merged_dt <- rbind(city_year_dt, urban_in_sub)
...
hist_plot <- ggplot(merged_dt_Seasonalityindex, aes(x = Seasonalityindex)) +
  ...
  scale_x_continuous(limits = c(0, 0.1), breaks = seq(0, 0.1, by =0.02)) +
  scale_y_continuous(name = "  ", limits = c(0, 320),
